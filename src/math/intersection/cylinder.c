...
int cylinder_int(t_line ray, const void *data, double *t)
{
    ...
    t_vec3 axis = vec3_normalise(cyl->axis);
    t_vec3 delta = vec3_sub(ray.origin, cyl->pos);

    t_vec3 a = vec3_sub(ray.dir, vec3_scalar_mul(axis, vec3_dot_product(ray.dir, axis)));
    ...
    double t_side = (t0 > EPSILON) ? t0 : ((t1 > EPSILON) ? t1 : INFINITY);

    if (cyl->height > 0.0 && t_side < INFINITY)
    {
        t_vec3 P = vec3_add(ray.origin, vec3_scalar_mul(ray.dir, t_side));
        double d = vec3_dot_product(vec3_sub(P, cyl->pos), axis);
        if (d < -EPSILON || d > cyl->height + EPSILON)
            t_side = INFINITY;
    }

    double t_bottom = INFINITY, t_top = INFINITY;
    t_vec3 cap_bottom = cyl->pos;
    t_vec3 cap_top = vec3_add(cyl->pos, vec3_scalar_mul(axis, cyl->height));

    double denom = vec3_dot_product(ray.dir, axis);
    if (fabs(denom) > EPSILON)
    {
        double t_cap = vec3_dot_product(vec3_sub(cap_bottom, ray.origin), axis) / denom;
        if (t_cap > EPSILON)
        {
            t_vec3 P = vec3_add(ray.origin, vec3_scalar_mul(ray.dir, t_cap));
    ...
    double t_final = fmin(fmin(t_side, t_bottom), t_top);
    if (t_final == INFINITY)
        return 0;
